...
2. Point to all relevant files and functions that need to be changed or created
...
Unless explicitly told to use a specific library, if there is a need to install a library then present the top 2 choices to the user along with a concise description of pros and cons for each before writing the plan. If you do so, incorporate the user's selection into the plan.
...
Using 2 - 5 words describe the feature, write the plan into an PLAN_<N>_<DESCRIPTION>.md file under "{git_root}/docs/featurePlans/" with the next available feature number
