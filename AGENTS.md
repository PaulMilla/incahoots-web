...
- **UserEventInfo**: Maps user to the event IDs they are associated with
...
- `getUserEventsPublisher(userId)` - Observable that combines user's events with their attendee details. Attendee records are now keyed by `userId` directly at path `events/{eventId}/eventAttendees/{userId}`.
